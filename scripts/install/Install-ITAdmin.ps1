...
.DESCRIPTION
    ...
    Sequence:
      preflight -> collect environment -> validate artifact -> stage -> provision database
                -> configure machine -> migrate -> configure IIS/HTTPS -> activate
...
.PARAMETER DatabaseHost
    PostgreSQL server host name or address. ITAdmin does not install PostgreSQL, but it does create
    its own database and its own least-privilege login role - see -DatabaseUser and -DatabaseAdminUser.

.PARAMETER DatabaseUser
    Name of the login role ITAdmin runs as. The role is created if it does not exist; its password
    is generated by the installer, stored only in the DPAPI machine secret store, and shown once at
    the end of the run. Supply -DatabasePassword only to manage the role's password externally.

.PARAMETER DatabaseAdminUser
    A PostgreSQL administrator (superuser, or a role with CREATEROLE and CREATEDB). Used only to
    create the database, the login role, and the schema grants. It is written to an ACL-restricted
    input file that is deleted immediately after use, is never stored, and never reaches runtime
    configuration. On a repair or update an existing configured connection is left untouched unless
    this is supplied again.

.PARAMETER DatabaseAdminPassword
    Password for -DatabaseAdminUser. Prompted for (never echoed) when omitted in interactive mode.

.PARAMETER DatabaseAdminDatabase
    Maintenance database to connect to as the administrator while creating the role and database.
...
# Generated application-role password. Established by Invoke-DatabaseProvisioning, consumed by
# Resolve-DatabaseConnectionString, and shown once in the installation summary when this run
# generated it. Held in a script variable so it is never written to a file or a log.
...
function Resolve-DatabaseConnectionString {
    <#
        Builds the Npgsql connection string for the application login role. The password is the one
        established by Invoke-DatabaseProvisioning (generated, or the explicit -DatabasePassword
        override); it is never written to the config file or the installation state, only to the
        DPAPI-protected machine secret store under ProgramData.
    #>
    param(
    ...
    $plain = $Script:DatabaseAppPassword
...
function New-DatabaseAppPassword {
    <#
        A generated PostgreSQL password for the application login role. Alphanumeric only: the value
        goes into a single-quoted SQL literal (CREATE ROLE ... PASSWORD '...') and into an Npgsql
        connection string, and restricting the alphabet keeps both unambiguous without escaping.
        32 characters from the OS CSPRNG.
    #>
    param([int]$Length = 32)
...
function Invoke-DatabaseProvisioning {
    <#
        Creates the ITAdmin database and its least-privilege login role, then records the generated
        application-role password on the script scope so Resolve-DatabaseConnectionString can build
        the runtime connection string with it.

        The operator supplies a PostgreSQL administrator credential (superuser, or a role with
        CREATEROLE and CREATEDB). It is used only here: it is written to an ACL-restricted input
        file that is removed in a finally block, is handed to ITAdmin.Api.exe --provision-database,
        and never reaches runtime configuration or the machine secret store. Only the generated
        application-role password is persisted.

        On a repair or update the machine secret store already carries a working connection string;
        provisioning is skipped unless the operator explicitly supplies an admin credential again.
    #>
    param(
        [Parameter(Mandatory = $true)][psobject]$Config,
    ...
    $Script:CurrentStep = "ProvisionDatabase"
    Set-Phase -State $State -Phase "Configuring"
    Write-Step "Provisioning the database and application role"

    $existingSecrets = Read-MachineSecrets
    $haveStoredConnection = ($null -ne $existingSecrets -and
        -not [string]::IsNullOrWhiteSpace($existingSecrets.connectionString))

    if ($haveStoredConnection -and $null -eq $DatabaseAdminPassword -and
        [string]::IsNullOrWhiteSpace($DatabaseAdminUser)) {
        Write-Detail "A configured database connection already exists; leaving the database and role untouched."
        $Script:DatabaseAppPassword = $null
        $Script:DatabaseAppPasswordGenerated = $false
        return
    }

    # The application role's password is generated, never chosen. -DatabasePassword stays as an
    # explicit bring-your-own override for sites that manage the role's password externally.
    if ($null -ne $DatabasePassword) {
        $appPassword = ConvertFrom-SecureStringToPlainText -Secure $DatabasePassword
        $Script:DatabaseAppPasswordGenerated = $false
        Write-Detail "Using the supplied application-role password (-DatabasePassword)."
    }
    else {
        $appPassword = New-DatabaseAppPassword
        $Script:DatabaseAppPasswordGenerated = $true
        Write-Detail "Generated an application-role password (32 chars, CSPRNG)."
    }
    $Script:DatabaseAppPassword = $appPassword

    $adminUser = Resolve-RequiredValue -Supplied $DatabaseAdminUser -Existing $null `
        -Prompt "PostgreSQL administrator user (used only to create the database and role)" `
        -Name "DatabaseAdminUser"

    $adminSecure = $DatabaseAdminPassword
    if ($null -eq $adminSecure) {
        if ($Unattended.IsPresent) {
            throw "-DatabaseAdminPassword is required in unattended mode."
        }
    ...
    if ([string]::IsNullOrWhiteSpace($adminPlain)) {
        throw "The PostgreSQL administrator password is required."
    }
    ...
    if (-not (Test-Path -LiteralPath $executable)) {
        throw "Provisioning host not found in the release payload: $executable"
    }
    ...
    try {
        ...
        $output = & $executable --provision-database --input $inputPath 2>&1
        $exitCode = $LASTEXITCODE

        if ($exitCode -eq 4) {
            foreach ($line in $output) { Write-Fail "$line" }
            throw "PostgreSQL accepted the administrator credential but the database precondition is " +
                  "still not met. Correct the values and re-run; nothing further was changed."
        }
        if ($exitCode -ne 0) {
            foreach ($line in $output) { Write-Fail "$line" }
        ...
        if ($resultLine.Count -gt 0) {
            ...
            $changes = @()
            if ($result.roleCreated) { $changes += "role created" }
            elseif ($result.roleUpdated) { $changes += "role password set" }
            if ($result.databaseCreated) { $changes += "database created" }
            if ($changes.Count -eq 0) { $changes += "already provisioned" }
            Write-Ok ("Database '{0}' / role '{1}' ready ({2})" -f `
                $Config.database.name, $Config.database.username, ($changes -join ", "))
        }
        else {
            Write-Ok "Database and application role are ready."
        }
    }
    finally {
...
# live in the application's own setup service, reached only through
# ITAdmin.Api.exe --bootstrap-directory (there is no in-application setup wizard). This script
# gathers input, proves the directory answers, and calls it.
# --------------------------------------------------------------------------------------------
...
function Write-InstallationSummary {
    <#
        ...
        printed; secret values are not - with one deliberate exception.

        The JWT signing key and the setup key exist only because ITAdmin needs them; nobody chose
        them and nobody has a reason to know them, so printing them would only copy them into a
        console buffer, a transcript, and probably a ticket. The generated database password is
        different: the operator never chose it either, but their DBA and their backup/restore
        runbook genuinely need it. It is therefore shown exactly once here, only when this run
        generated it, and never written to a log or a file in clear text.
    #>
    param(
        [Parameter(Mandatory = $true)][psobject]$Config,
        [Parameter(Mandatory = $true)][psobject]$Artifact,
    ...
    if ($Script:DatabaseAppPasswordGenerated -and
        -not [string]::IsNullOrWhiteSpace($Script:DatabaseAppPassword) -and
        -not $Unattended.IsPresent) {
        ...
        Write-Host "    store listed above. It is not written to any log or file in clear text and"
        Write-Host "    will not be shown again. Record it in your database and backup runbooks."
        Write-Host "  ----------------------------------------------------------------------" -ForegroundColor Yellow
        Write-Host ""
    }
...
try {
    ...
    Invoke-DatabaseProvisioning -Config $environmentConfig -PayloadRoot $releasePaths.PayloadRoot -State $state
