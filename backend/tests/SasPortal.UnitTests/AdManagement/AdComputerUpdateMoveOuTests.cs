...
public sealed class AdComputerUpdateMoveOuTests
{
    ...
    public void ExistingDeploymentMigration_SeedsComputerUpdateAndMoveOuPermissionsIdempotently()
    {
        var migrationSource = File.ReadAllText(
            Path.Combine(
                FindRepositoryRoot(),
                "backend/src/SasPortal.Persistence/Migrations/20260612130000_SeedAdManagementComputersUpdateMoveOuPermissions.cs"));
    ...
    public void ComputerMoveOuService_ValidatesTargetOuAndPreservesRdn()
    {
        var source = File.ReadAllText(
            Path.Combine(
                FindRepositoryRoot(),
                "backend/src/SasPortal.Infrastructure/Services/AdUserDirectoryService.ComputerMoveOu.cs"));
