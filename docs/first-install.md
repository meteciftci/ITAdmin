...
PostgreSQL must already be running and reachable from the ITAdmin server. That is the whole
precondition — ITAdmin does not install PostgreSQL, but it **does** create its own database and its
own least-privilege login role.

At install time you supply:

- **PostgreSQL user** — the name of the login role ITAdmin runs as (for example `itadmin_app`).
  Created if it does not exist. Its password is **generated by the installer**, stored only in the
...
Under the covers, before any machine change, the installer runs
`ITAdmin.Api.exe --provision-database`: exit `0` provisioned and verified · `4` reachable but the
precondition could not be satisfied · `1` unreachable or input rejected. The step is idempotent, so
a re-run converges rather than erroring. To manage the application role's password outside the
installer, pass `-DatabasePassword` and it is set to that value instead of being generated.
...
9. Prompts for the PostgreSQL target, a transient PostgreSQL administrator credential, the
   directory bind, and the initial administrator.
10. Stages the versioned release, provisions the database and application role, configures the
    machine, applies migrations, establishes the Primary Directory and initial administrator
    (`--bootstrap-directory` — there is no in-application setup wizard), configures IIS, activates,
    and health-checks.
...
PostgreSQL administrator password for '<admin>'
...
installer generated it, i.e. you did not pass `-DatabasePassword`). Record it: the DPAPI secret
store does not travel to another host, so a bare-metal restore needs that value.
...
ITAdmin installation completed successfully.
  ...
  ----------------------------------------------------------------------
    ...
    ... stored in the DPAPI secret store; not written to any log;
    will not be shown again.
  ----------------------------------------------------------------------
```

The one-time block appears only when the installer generated the application role's password (not
when you passed `-DatabasePassword`).
