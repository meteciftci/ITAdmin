...
import type { DnsAuthenticationMode, DnsConnectionTransport, DnsCredentialProfile, DnsServer, DnsServerConnectionTest, DnsServerEnvironment } from "./types";

const emptyCredential = { name: "", userName: "", password: "", authenticationMode: "Negotiate" as DnsAuthenticationMode, isEnabled: true };
const emptyServer = { displayName: "", hostName: "", port: 5985, transport: "Http" as DnsConnectionTransport, environment: "Internal" as DnsServerEnvironment, credentialProfileId: "", isEnabled: true, syncIntervalMinutes: null as number | null, tlsCertificateThumbprint: "", notes: "" };
  ...
  return <section className="space-y-4">
        ...
        <table className="w-full text-sm">
          ...
          <tbody>{servers.data?.map((x) => {
            ...
            return <tr key={x.id} className="border-b last:border-0">
              <td className="p-3 font-medium">{x.displayName}</td><td className="p-3"><span className="mr-2 text-xs uppercase text-muted-foreground">{x.transport}</span>{x.hostName}:{x.port}</td>
    ...
    {canManage ? <>
      ...
      <SectionCard title={serverId ? t("servers.editTitle") : t("servers.addTitle")}>
        <div className="space-y-4"><div className="grid gap-4 md:grid-cols-2"><div className="space-y-2"><Label>{t("servers.fields.name")}</Label><Input value={server.displayName} onChange={(e) => setServer({ ...server, displayName: e.target.value })} /></div><div className="space-y-2"><Label>{t("servers.fields.hostName")}</Label><Input value={server.hostName} onChange={(e) => setServer({ ...server, hostName: e.target.value })} /></div><div className="space-y-2"><Label>{t("servers.fields.transport")}</Label><Select value={server.transport} onChange={(e) => { const transport = e.target.value as DnsConnectionTransport; setServer({ ...server, transport, port: transport === "Http" ? 5985 : 5986, tlsCertificateThumbprint: transport === "Http" ? "" : server.tlsCertificateThumbprint, credentialProfileId: "" }); }}><option value="Https">{t("servers.transports.Https")}</option><option value="Http">{t("servers.transports.Http")}</option></Select></div><div className="space-y-2"><Label>{t("servers.fields.port")}</Label><Input type="number" min="1" max="65535" value={server.port} onChange={(e) => setServer({ ...server, port: Number(e.target.value) })} /></div><div className="space-y-2"><Label>{t("servers.fields.environment")}</Label><Select value={server.environment} onChange={(e) => setServer({ ...server, environment: e.target.value as DnsServerEnvironment })}><option value="Internal">{t("environments.Internal")}</option><option value="Public">{t("environments.Public")}</option><option value="Other">{t("environments.Other")}</option></Select></div><div className="space-y-2"><Label>{t("servers.fields.credential")}</Label><Select value={server.credentialProfileId} onChange={(e) => setServer({ ...server, credentialProfileId: e.target.value })}><option value="">{t("servers.selectCredential")}</option>{credentials.data?.filter((x) => x.isEnabled && (server.transport === "Https" || x.authenticationMode === "Negotiate")).map((x) => <option key={x.id} value={x.id}>{x.name}</option>)}</Select></div><div className="space-y-2"><Label>{t("servers.fields.syncInterval")}</Label><Input type="number" min="1" max="1440" value={server.syncIntervalMinutes ?? ""} onChange={(e) => setServer({ ...server, syncIntervalMinutes: e.target.value ? Number(e.target.value) : null })} /></div>{server.transport === "Https" ? <div className="space-y-2 md:col-span-2"><Label>{t("servers.fields.thumbprint")}</Label><Input value={server.tlsCertificateThumbprint} onChange={(e) => setServer({ ...server, tlsCertificateThumbprint: e.target.value })} /></div> : <p className="text-sm text-muted-foreground md:col-span-2">{t("servers.httpSecurityNotice")}</p>}<div className="space-y-2 md:col-span-2"><Label>{t("servers.fields.notes")}</Label><Textarea value={server.notes} onChange={(e) => setServer({ ...server, notes: e.target.value })} /></div><CheckboxField id="server-enabled" label={t("servers.fields.enabled")} checked={server.isEnabled} onCheckedChange={(x) => setServer({ ...server, isEnabled: x === true })} /></div><div className="flex justify-end gap-2">{serverId ? <Button variant="outline" onClick={() => { setServerId(null); setServer(emptyServer); }}>{t("common:actions.cancel")}</Button> : null}<Button onClick={() => serverMutation.mutate()} disabled={!server.displayName || !server.hostName || !server.credentialProfileId}>{t("common:actions.save")}</Button></div></div>
      </SectionCard>
