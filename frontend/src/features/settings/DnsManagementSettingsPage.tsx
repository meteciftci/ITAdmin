...
import {
  DNS_SETTINGS_QUERY_KEY,
  getDnsSettings,
  updateDnsSettings,
} from "@/features/dns-management/api";
...
const defaults: DnsManagementSettings = {
  ...
  maxParallelServers: 3,
  snapshotRetentionDays: 30,
  syncRecordInventory: true,
  promptForFullSyncOnComparisonOpen: true,
...
export function DnsManagementSettingsPage() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      setError(null);
      await queryClient.invalidateQueries({ queryKey: DNS_SETTINGS_QUERY_KEY });
      toast.success(t("settings.saved"));
    },
    onError: (x) => setError(getApiErrorMessage(x, t("settings.saveFailed"))),
  });
  const numberField = (
    key: keyof DnsManagementSettings,
    min: number,
    max: number,
  ) => (
    <div className="space-y-2">
      <Label htmlFor={String(key)}>{t(`settings.fields.${String(key)}`)}</Label>
      <Input
        id={String(key)}
        type="number"
        min={min}
  ...
  return (
    <section className="space-y-4">
      ...
      {query.isError ? (
        <FormError
          message={getApiErrorMessage(query.error, t("settings.loadFailed"))}
        />
      ) : null}
      <SectionCard
        title={t("settings.moduleTitle")}
        description={t("settings.moduleDescription")}
      >
        <div className="space-y-5">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <SwitchField
              id="dns-enabled"
              label={t("settings.fields.isEnabled")}
              checked={form.isEnabled}
              onCheckedChange={(x) =>
                setForm({ ...form, isEnabled: x === true })
              }
            />
            <SwitchField
              id="dns-auto-sync"
              label={t("settings.fields.automaticSyncEnabled")}
              checked={form.automaticSyncEnabled}
              onCheckedChange={(x) =>
                setForm({ ...form, automaticSyncEnabled: x === true })
              }
            />
            <SwitchField
              id="dns-records"
              label={t("settings.fields.syncRecordInventory")}
              checked={form.syncRecordInventory}
              onCheckedChange={(x) =>
                setForm({ ...form, syncRecordInventory: x === true })
              }
            />
            <SwitchField
              id="dns-prompt"
              label={t("settings.fields.promptForFullSyncOnComparisonOpen")}
              checked={form.promptForFullSyncOnComparisonOpen}
              onCheckedChange={(x) =>
                setForm({
                  ...form,
                  promptForFullSyncOnComparisonOpen: x === true,
                })
              }
            />
