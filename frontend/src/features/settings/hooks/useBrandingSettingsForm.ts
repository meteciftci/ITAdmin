...
export type UseBrandingSettingsFormReturn = {
  ...
  hydrateFromBranding: (branding: BrandingSettings) => void;
...
export function useBrandingSettingsForm({
  ...
  const hydrateFromBranding = useCallback((branding: BrandingSettings) => {
  ...
  const validateBrandingInput = useCallback((): boolean => {
    ...
    return true;
  }, [brandingApplicationName, brandingBrowserTitle, t]);
