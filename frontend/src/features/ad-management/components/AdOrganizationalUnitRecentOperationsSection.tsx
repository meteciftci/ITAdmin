...
export function AdOrganizationalUnitRecentOperationsSection({
  ...
  const logsQuery = useQuery({
    ...
    queryFn: () =>
      getAdOperationLogs({
        targetObjectGuid: organizationalUnitId,
        pageNumber: 1,
  ...
  return (
    <>
      <SectionCard
        title={t("adManagement:organizationalUnits.detail.recentOperations")}
        actions={
          <Link
            to={buildAdOperationLogsPath(organizationalUnitId)}
            className={cn(buttonVariants({ variant: "outline", size: "sm" }))}
          >
            {t("adManagement:organizationalUnits.detail.viewAllLogs")}
          </Link>
        }
      >
        ...
        {logsQuery.isError ? (
          <p className="text-sm text-muted-foreground">
            {t("adManagement:organizationalUnits.errors.loadFailed")}
          </p>
        ) : null}

        {logsQuery.isSuccess && logs.length === 0 ? (
          <p className="text-sm text-muted-foreground">
            {t("adManagement:organizationalUnits.detail.noRecentOperations")}
          </p>
        ) : null}
              ...
              <thead className="bg-muted/50 text-left">
                <tr>
                  ...
                  <th className="px-3 py-2 font-medium whitespace-nowrap text-right">
                    {t("adManagement:organizationalUnits.detail.operationDetail")}
                  </th>
                </tr>
              </thead>
              <tbody>
