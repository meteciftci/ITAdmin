...
export function LicensePackageEditPage() {
  ...
  return (
    <section className="mx-auto w-full max-w-7xl space-y-4">
      <PageHeader
        title={t("licenseManagement:pages.packages.edit.title")}
        description={detailQuery.data?.productName}
        actions={
          <Link to={buildLicensePackageDetailPath(id)} className={cn(buttonVariants({ variant: "outline" }))}>
      ...
      {detailQuery.data ? (
        <SectionCard title={t("licenseManagement:pages.packages.edit.formTitle")}>
          <LicensePackageForm
            mode="edit"
            packageItem={detailQuery.data}
            onCancel={() => navigate(buildLicensePackageDetailPath(id))}
            onSaved={() => {
              ...
              toast.success(t("licenseManagement:messages.packageUpdated"));
              navigate(buildLicensePackageDetailPath(id));
            }}
          />
        </SectionCard>
      ) : null}
