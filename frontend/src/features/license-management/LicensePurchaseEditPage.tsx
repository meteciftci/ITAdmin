...
export function LicensePurchaseEditPage() {
  ...
  return (
    <section className="mx-auto w-full max-w-7xl space-y-4">
      <PageHeader
        title={t("licenseManagement:pages.purchases.edit.title")}
        description={detailQuery.data?.title}
        actions={
          <Link to={buildLicensePurchaseDetailPath(id)} className={cn(buttonVariants({ variant: "outline" }))}>
      ...
      {detailQuery.data ? (
        <SectionCard title={t("licenseManagement:pages.purchases.edit.formTitle")}>
          <LicensePurchaseForm
            mode="edit"
